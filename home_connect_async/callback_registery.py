...
class CallbackRegistry:
    """Manage callback registration and notifications."""
    ...
    def register_callback(
        self,
        callback: Callable[[Appliance, str, Any], None]
        | Callable[[Appliance, str], None]
        | Callable[[Appliance], None]
        | Callable[[], None],
        ...
        * appliance - An optional appliance object or haId to filter the events for
        ...
        for key in keys:
            if "*" in key:
                ...
                if not self.wildcard_registered(
                    callback_record, self._callbacks[haid][self.WILDCARD_KEY]
                ):
    ...
    def deregister_callback(
        ...
        for key in keys:
            if "*" in key:
                if (
                    haid in self._callbacks
                    and self.WILDCARD_KEY in self._callbacks[haid]
                ):
                    new_list = [
                        item
                        for item in self._callbacks[haid][self.WILDCARD_KEY]
                        if item["key"] != key or item["callback"] != callback
                    ]
    ...
    def wildcard_registered(self, callback_record, callback_list) -> bool:
        ...
        for item in callback_list:
            if (
                item["key"] == callback_record["key"]
                and item["callback"] == callback_record["callback"]
            ):
                return True
        return False
    ...
    async def async_broadcast_event(
        self, appliance: Appliance, event_key: str | Events, value: any = None
    ) -> None:
        ...
        handlers = [
            handler for handler in [None, appliance.haId] if handler in self._callbacks
        ]
    ...
    async def _async_call(
        ...
        try:
            ...
            _LOGGER.warning(
                "Unhandled exception in callback function for event_key: %s",
                event_key,
                exc_info=ex,
            )
        if callback_error:
