...
class AbstractAuth(ABC):
    ...
    This is a pattern required by Home Assistant.
    """
    ...
    async def request(
        self, method, endpoint: str, lang: str = None, **kwargs
    ) -> ClientResponse:
        """Make a request."""
    ...
    async def stream(
        self, endpoint: str, lang: str = None, **kwargs
...
class AuthManager(AbstractAuth):
    """
    Implements a full fledged authentication manager when the SDK is not being used by Home Assistant.
    """
    ...
    def get_access_token(self):
        ...
        if (
            self._cm.access_token_expirs_at
            and datetime.now() > self._cm.access_token_expirs_at
        ):
            self.renew_token()
...
class HomeConnectCredentialsManager(CredentialManager):
    ...
    def __init__(
        self, service_information: ServiceInformation, proxies: dict | None = None
    ):
    ...
    def _process_token_response(
        ...
        if "expires_in" in token_response:
            self.access_token_expirs_at = datetime.now() + timedelta(
