...
class HomeConnect(DataClassJsonMixin):
    ...
    _api: Optional[HomeConnectApi] = field(
        default=None,
        metadata=config(encoder=lambda val: None, exclude=lambda val: True),
    )
    _updates_task: Optional[Task] = field(
        default=None,
        metadata=config(encoder=lambda val: None, exclude=lambda val: True),
    )
    _load_task: Optional[Task] = field(
        default=None,
        metadata=config(encoder=lambda val: None, exclude=lambda val: True),
    )
    _callbacks: Optional[CallbackRegistry] = field(
        default_factory=lambda: CallbackRegistry(),
        metadata=config(encoder=lambda val: None, exclude=lambda val: True),
    )
        ...
        * auto_update - Subscribe for real-time updates to the data model, ignored for delayed_load
        ...
        if json_data:
            try:
                ...
                _LOGGER.exception(
                    "Exception when loading HomeConnect data from JSON", exc_info=ex
                )
        if not hc:
            hc = HomeConnect()
    ...
    def start_load_data_task(
        ...
        * refresh - optional refresh mode, if not supplied the value from async_create() will be used
        ...
        self._load_task = asyncio.create_task(
            self.async_load_data(refresh, on_complete, on_error),
            name="_async_load_data",
        )
        ...
        try:
            if refresh == self.RefreshMode.NOTHING:
                ...
                if response.status != 200:
                    _LOGGER.warning(
                        "Failed to get the list of appliances code=%d error=%s",
                        response.status,
                        response.error_key,
                    )
                    raise HomeConnectError(
                        f"Failed to get the list of appliances (code={response.status})",
                        response=response,
                    )
                data = response.data
                        ...
                        if ha["connected"]:
                            if haid in self.appliances:
                                ...
                                if refresh == self.RefreshMode.DYNAMIC_ONLY:
                                    await self.appliances[haid].async_fetch_data(
                            ...
                            await self._callbacks.async_broadcast_event(
                                self.appliances[ha["haId"]], Events.PAIRED
                            )
                            await self._callbacks.async_broadcast_event(
                                self.appliances[ha["haId"]], Events.DATA_CHANGED
                            )
                            _LOGGER.debug(
                                "Loadded appliance: %s",
                                self.appliances[ha["haId"]].name,
                            )
                        elif haid in self.appliances:
                            _LOGGER.warning(
                                "The appliance (%s) is disconnected when loading for the first time",
                                haid,
                            )
                            await self.appliances[haid].async_set_connection_state(
                                False
                            )
                ...
                for haId in self.appliances.keys():
                    if haId not in haid_list:
                        await self._callbacks.async_broadcast_event(
                            self.appliances[haId], Events.DEPAIRED
                        )
            ...
            _LOGGER.warning(
                "Failed to load data from Home Connect (%s)", str(ex), exc_info=ex
            )
    ...
    def subscribe_for_updates(self):
        ...
        if not self._updates_task:
            ...
            self._updates_task = asyncio.create_task(
                self.async_events_stream(), name="subscribe_for_updates"
            )
    ...
    async def async_events_stream(self):
        ...
        while True:
            try:
                ...
                event_source = await self._api.async_get_event_stream(
                ...
                _LOGGER.debug(
                    "ConnectionRefusedError in SSE connection refused. Will try again",
                    exc_info=ex,
                )
                ...
                if error_code == 429:
                    ...
                    if backoff > 3600:
                        ...
                        backoff = 60
                    _LOGGER.debug(
                        "Got error 429 when opening event stream connection, will sleep for %s seconds and retry",
                        backoff,
                    )
                else:
                    _LOGGER.debug(
                        "ConnectionError in SSE event stream. Will wait for %d seconds and retry ",
                        backoff,
                        exc_info=ex,
                    )
                    backoff *= 2
                ...
                GlobalStatus.unset_status(GlobalStatus.Status.UPDATES)
                _LOGGER.debug(
                    "Exception in SSE event stream. Will wait for %d seconds and retry ",
                    backoff,
                    exc_info=ex,
                )
                await asyncio.sleep(backoff)
    ...
    async def _async_process_updates(self, event: MessageEvent):
        ...
        if event.type == "PAIRED":
            ...
            if haid in self.appliances:
                ...
                await self._callbacks.async_broadcast_event(
                    self.appliances[haid], Events.DISCONNECTED
                )
        elif event.type == "CONNECTED":
            if haid in self.appliances:
                await self.appliances[haid].async_set_connection_state(True)
                await self._callbacks.async_broadcast_event(
                    self.appliances[haid], Events.CONNECTED
                )
            else:
                self.appliances[haid] = await Appliance.async_create(self, haId=haid)
                await self._callbacks.async_broadcast_event(
    ...
    def register_callback(
        self,
        callback: Callable[[Appliance, str], None]
        | Callable[[Appliance, str, any], None],
