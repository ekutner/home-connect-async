        ...
        if haid in self._callbacks:
            if event_key in self._callbacks[haid]:
                for callback in self._callbacks[haid][event_key]:
                    await self._async_call(callback, event_key, value)
                    handled = True
            ...
            if not handled and DEFAULT_KEY in self._callbacks[haid]:
                for callback in self._callbacks[DEFAULT_KEY]:
                    self._async_call(callback, event_key, value)


    async def _async_call(self, callback:Callable, event_key:str, value:any) -> None:
        """ Helper funtion to make the right kind of call to the callback funtion """
        sig = inspect.signature(callback)
        param_count = len(sig.parameters)
        if inspect.iscoroutinefunction(callback):
            if param_count == 3:
                await callback(self, event_key, value)
            elif param_count == 2:
                await callback(self, event_key)
            elif param_count == 1:
                await callback(self, event_key)
            else:
            ...
            if param_count == 3:
                callback(self, event_key, value)
            elif param_count == 2:
                callback(self, event_key)
            elif param_count == 1:
                callback(self, event_key)
            else:
                raise ValueError(f"Unexpected number of callback parameters: {sig}")
